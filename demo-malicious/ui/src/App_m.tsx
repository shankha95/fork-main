...
const baseURL = "http://192.168.2.125:4001"
...
function App() {
  ...
  // Malicious Single Cluster Handler
  const maliciousSingleClusterHandler = async () => {
    setSingleClusterLoading(true)
    const res = await axios.post(`${baseURL}/malicious/single`) // Updated endpoint
    if (res.status === 200) {
  ...
  // Malicious Multi-Cluster Handler
  const maliciousMultiClusterHandler = async () => {
    setMultiClusterLoading(true)
    const res = await axios.post(`${baseURL}/malicious/multi`) // Updated endpoint
    if (res.status === 200) {
          ...
          <Flex direction='column' mt='4rem'>
            <Flex direction='column' mb='3.5rem' gap='lg'>
              <Button
                ...
                onClick={maliciousSingleClusterHandler}>
            ...
            <Flex direction='column' gap='lg'>
              <Button loading={multiClusterLoading} onClick={maliciousMultiClusterHandler}>
                Vegeta (Multi cluster)              </Button>
